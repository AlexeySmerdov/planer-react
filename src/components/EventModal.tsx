...
interface EventModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (eventData: NewEventForm) => void;
  selectedDate?: string;
  selectedTime?: string;
...
export const EventModal: React.FC<EventModalProps> = ({
  ...
  const [formData, setFormData] = useState<NewEventForm>({
    ...
    date: selectedDate || '',
    time: selectedTime || '',
    duration: selectedDuration || '30 минут',
    category: 'work',
    description: '',
    participants: []
  });
        ...
        <div className="bg-gradient-to-r from-indigo-500 to-blue-500 p-6 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-semibold">Новое событие</h2>
                <p className="text-white/80 text-sm">Создайте событие в календаре</p>
              </div>
            </div>
            <button
              onClick={handleClose}
